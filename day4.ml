...
type input = int list * (int * bool) array array list
...
let parse_board lines =
  let parse_line s =
    ...
    |> Array.of_list
  in
  List.tl_exn lines |> List.map ~f:parse_line |> Array.of_list
...
let mark_board n board =
  Array.map board ~f:(fun row ->
      Array.map row ~f:(fun cell -> if n = fst cell then (n, true) else cell))
...
let calc_score board =
  ...
  Array.to_list board |> Array.concat
...
let part1 (numbers, boards) =
  let board, num = find_first_bingo boards numbers in
  calc_score board * num

let part2 (numbers, boards) =
  let board, num = find_last_bingo boards numbers in
  calc_score board * num

...
let run () = 
  let nums = In_channel.read_lines "input/day3.txt" |> List.map ~f:parse_binary in
  let gamma = calc_gamma nums in
  let epsilon = calc_epsilon nums in
  let oxygen = calc_oxygen nums in
  let co2 = calc_co2 nums in
  printf "Gamma %d Epsilon %d -> %d\n" gamma epsilon (gamma * epsilon);
  printf "Oxygen %d CO2 %d -> %d\n" oxygen co2 (oxygen * co2)

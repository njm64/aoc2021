...
let rec inc_energy m x y =
  let e = m.(y).(x) in
  if e < 10 then m.(y).(x) <- e + 1;
  if e + 1 = 10 then for_neighbours m x y inc_energy

(* If the octopus at (x,y) has flashed, reset it *)
let check_reset m x y = if m.(y).(x) = 10 then m.(y).(x) <- 0
...
let count_until_all_flashed m =
  let rec next m i =
    let m, count = step m in
    if count = width m * height m then i else next m (i + 1)
  in
  next m 1

let part1 m = count_flashes m 100
let part2 m = count_until_all_flashed m
